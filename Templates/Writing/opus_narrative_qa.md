...
Do not read, apply, reproduce, score, or report automatically learned channel script rules. This boundary is SILENT: do not mention channel rules, their presence or absence, compliance with this boundary, deleted rule files, or analytics-rule status anywhere in the output. Do not output an `Active Channel Rule Compliance` section, any equivalent learned-rule compliance table, or a sentence such as `No ACTIVE channel script rules` / `no learned channel rules were applied`. Channel analytics are observational only for Narrative QA. Channel-specific writing rules may affect this gate only when they have been deliberately added through the approved manual rule path. Continue to apply the normal Hook, retention, semantic-progression, repetition, transition, safety, blueprint, and CTA checks in this prompt.
