...
def loser_repair_card(*,audit_id:int,title:str,original_text:str,source_thumbnail_path:str,why:list[dict[str,Any]],suggestion:dict[str,Any],proposed_text:str,original_visual:dict[str,Any]|None=None,target_visual:dict[str,Any]|None=None,human_prompt_override:str|None=None)->dict[str,Any]:
    ...
    target=target_visual or {}
    audit_edit_contract={"audit_id":audit_id,"aspect_ratio":"16:9","proposed_thumbnail_text":proposed_text}
    for key in ("human_count","face_count","safe_zone_clear","presenter_position","text_zone","hero_count"):
        if key in target: audit_edit_contract[key]=target[key]
    if target.get("forbidden_items") is not None: audit_edit_contract["forbidden_items"]=list(target.get("forbidden_items") or [])
    if target.get("repair_checks") is not None: audit_edit_contract["repair_checks"]=list(target.get("repair_checks") or [])
    return {"audit_id":audit_id,"title":title,"source_thumbnail_path":source_thumbnail_path,"original_thumbnail_text_full":original_text,"why_lower_ctr":why,"suggestion_for_loser":suggestion,"proposed_thumbnail_text":proposed_text,"generated_edit_prompt_original":auto_prompt,"editable_image_prompt":effective_prompt,"prompt_human_edited":human_prompt_override is not None,"verifier_contract_status":"requires_structured_review" if human_prompt_override is not None else "aligned_with_generated_prompt","audit_edit_contract":audit_edit_contract,"generate_now":{"enabled":bool(source_thumbnail_path.strip()),"button_label":"Generate Now","request_preview":request.payload(),"final_prompt_preview":final_generation_prompt(request),"status":"provider_required"}}
