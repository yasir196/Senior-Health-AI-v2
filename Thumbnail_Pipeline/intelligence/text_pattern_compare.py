...
def compare_text_patterns(rows:list[dict[str,Any]],min_impressions:int|None=None)->dict[str,Any]:
    cfg=load_settings(); floor=cfg["eligibility"]["min_impressions"] if min_impressions is None else min_impressions
    eligible=[]
    for row in rows:
        try:
            if float((row.get("performance") or {}).get("impressions"))>=float(floor): eligible.append(row)
        except (TypeError,ValueError): pass
    discovered=discover_text_patterns(eligible)
