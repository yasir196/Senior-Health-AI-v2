...
def validate_scene_segmentation(rows: list[dict[str, str]]) -> list[str]:
    ...
    Tiny fragments must normally be merged. Word count is a pacing target, not a
    hard maximum: complete approved sentences may exceed the target and remain intact.
    """
    issues: list[str] = []
    for idx, row in enumerate(rows, 1):
        sid = _clean(row.get("scene_id")) or f"row {idx}"
