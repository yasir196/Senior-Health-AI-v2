...
3A. **Bound-before-reject rule:** Gate 1 must adjudicate the narrowest source-supported educational proposition, not merely accept or reject the strongest proposed interpretation. When a verified source cannot support a direct benefit, treatment, prevention, diagnosis, causal, or target-outcome claim, explicitly test whether it still supports a materially useful narrower statement such as study context, task distinction, population/formulation distinction, association, null/mixed finding, limitation, safety fact, or mechanism/context that the source itself establishes. If that narrower statement is relevant to the immutable Stage-1 angle and medically safe, mark the claim `bounded` and write the exact narration-usable wording/boundary. Do not reject an entire verified evidence module solely because it cannot prove the video's strongest outcome claim.
3B. `bounded` may therefore mean **CONTEXT-ONLY / LIMITATION-ONLY**. Such evidence may educate or explain but must not be used as proof that the featured food, exercise, habit, or intervention produces the target outcome. Preserve the actual population, formulation/exposure, study design, uncertainty, and direction of evidence. Association must remain association; indirect evidence must remain indirect; supplement/extract/powder evidence must not be transferred to a food; chair-rise evidence must not be transferred to floor-rise; and a mechanism must not be presented as a demonstrated clinical outcome.
3C. Reject the proposition only when the source is unverified/unreliable for production use, the proposition remains unsupported even after reasonable narrowing, it is outside the approved Stage-1 angle, or safe narration would still create a prohibited/unsafe medical implication. Gate 1 must never weaken SYS_09 safety rules merely to preserve breadth.
4. Mark each claim in the human-readable log as approved, approved with wording limits/context-only, needs revision, or rejected. For every rejected material proposition, state why a narrower narration-usable boundary could not be approved.
