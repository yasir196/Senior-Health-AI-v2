...
- Verify cited material sources at source level before making them production-usable. Never invent a citation or silently strengthen the imported wording.
- **Imported-source verification fallback ladder:** a failed or blocked first URL is not, by itself, an unverified source. For every material imported citation, use identifiers already present in the report (title, authors, year, journal/body, DOI, PMID, PMCID, registry/guideline identifier, or cited URL) to attempt reasonable source-level verification through alternate authoritative representations before declaring it unverified. Prefer, as applicable: the original journal/publisher or official guideline/government/institution page; PubMed/PMC or another authoritative bibliographic record that exposes the study abstract/metadata; DOI resolution/metadata; and an official mirror or repository copy from the same issuing body. Search by exact title plus author/year when the imported URL is dead, blocked, or incomplete. A source may be verified from an authoritative alternate representation when identity is matched and that representation directly supports the proposition being normalized.
- Record verification provenance in the project-local source note/catalog: what authoritative representation was actually checked and whether verification was **full-text/source-level**, **abstract/metadata-level**, or **identity-only**. Identity-only verification proves that a source exists but does not make a substantive claim production-usable. Abstract/metadata-level verification may support only propositions explicitly visible there; do not infer unavailable methods, subgroup details, effect sizes, harms, doses, or conclusions. Full-text/source-level verification may support only what the checked source actually establishes.
- Mark a material imported source `unverified/not production-usable` only after reasonable fallback paths have been attempted or when the available authoritative representation does not expose enough content to support the proposition. Briefly record the attempted fallback path/reason. Do not substitute a different study merely because it reaches a similar conclusion, and do not use search-result snippets, AI summaries, blogs, or secondary paraphrases as source-level verification of a primary-study claim.
- Keep `DISCOVERED` evidence conceptually separate from `APPROVED` production claims. Indirect, observational, null, mixed, safety, mechanism/context, or formulation-distinction evidence may remain valuable educational context even when it cannot support a direct benefit claim. When a verified finding is relevant but cannot support the strongest target-outcome proposition, normalize the narrowest source-supported proposition as its own material claim so Medical Gate 1 can adjudicate it as bounded/context-only rather than forcing an all-or-nothing outcome. Never manufacture a weaker claim that the source itself does not support.
